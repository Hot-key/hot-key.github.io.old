---
layout: post
title:  "[WindowsForm] 더욱 둥근 테두리를 가진 폼 만들기-추가사항"
subtitle:   "부드러운 둥근 테두리 만들기"
categories: develop
tags: winforms
---

이전글 [[WindowsForm] 더욱 둥근 테두리를 가진 폼 만들기](https://hot-key.github.io/develop/2019/11/18/dev-winforms-smooth-rounded/)

이전글을 작성할때는 몰랐는데 작성이 끝나고 해당 기능을 사용하려고 보니 발생한 문제 가 있었다.

아마 위의 기능을 사용하려고 한 사람이면 다 발생했을 문제점이다.

이전글을 보면 투명한 폼을 사용하기 위하여 Per Pixel Alpha Blend를 이용한다.  
이를 이용하기 위해서 ``CreateParams.ExStyle`` 을 ``WS_EX_LAYERED(0x00080000)`` 으로 변경하였다.
...
위의 코드를 넣고 ``public partial class Form1 : NanoRoundedForm`` 이런식으로``NanoRoundedForm`` 을 상속받아 사용하면 된다.